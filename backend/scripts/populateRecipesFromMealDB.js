// backend\scripts\populateRecipesFromMealDB.js
...
let Recipe;
try {
  // First try to load existing Recipe model
  Recipe = mongoose.model('Recipe');
  console.log('Using existing Recipe model');
} catch (e) {
  // If not available, define a new one
...
const fetchMealDetails = async (mealId) => {
  try {
    ...
    console.error(`Error fetching details for meal ID ${mealId}:`, error.message);
    return null;
  }
};
...
const fetchMealsBySearchTerm = async (term) => {
  try {
    ...
    console.error(`Error fetching meals for search term ${term}:`, error.message);
    return [];
  }
};
...
const formatRecipe = (meal, idCounter) => {
...
const populateRecipes = async () => {
  try {
    ...
    console.log(`Target: Add recipes until we have at least ${targetCount} recipes`);
    let addedCount = 0;
    
    // Get available categories
    try {
      const categoriesResponse = await axios.get('https://www.themealdb.com/api/json/v1/1/categories.php');
      const categories = categoriesResponse.data.categories.map(cat => cat.strCategory);
      ...
      for (const category of categories) {
        ...
        for (const meal of meals) {
          try {
            ...
            const existingRecipe = await Recipe.findOne({ recipeId: parseInt(meal.idMeal) });
            ...
            if (currentCount >= targetCount) {
              console.log(`Reached target of ${targetCount} recipes (current: ${currentCount})`);
            ...
            console.error(`Error processing meal ${meal.strMeal}:`, error.message);
          }
        }
      }
      ...
      if (existingCount + addedCount < targetCount) {
        ...
        for (const letter of letters) {
          ...
          console.log(`Found ${meals.length} meals starting with letter ${letter}`);
      ...
      if (existingCount + addedCount < targetCount) {
        ...
        const searchTerms = [
          'chicken', 'beef', 'pork', 'fish', 'pasta', 'salad', 'soup', 'cake', 
          'cookie', 'pie', 'bread', 'rice', 'potato', 'vegetable', 'breakfast', 
          'dinner', 'lunch', 'dessert', 'appetizer', 'snack'
        ];
    ...
    console.log(`Recipe population completed! Added ${addedCount} new recipes.`);
